...
这个错误是由于数据库的 `sql_mode` 配置为 `only_full_group_by`，导致查询中的 ORDER BY 子句与 GROUP BY 子句的列不兼容。
...
在业务层面上修改代码，进行对数据列表排序，在结果视图 VO 里面写个比较器，然后使用。
...
public class TestServiceImpl extends BaseServiceImpl<TestMapper, TestEntity>
  ...
  public Page<OvertimeAlarmVO> overtimeAlarm(OvertimeAlarmReqDTO reqDTO) {
    ...
    overtimeAlarmList.sort(new OvertimeAlarmVO.TimeComparator());
